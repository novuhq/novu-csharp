        ...
        public async Task<InboundWebhooksControllerHandleWebhookResponse> TrackAsync(string environmentId, string integrationId, object requestBody, string? idempotencyKey = null, RetryConfig? retryConfig = null)
        {
            ...
            if(responseStatusCode == 200)
            {
                if(Utilities.IsContentTypeMatch("application/json", contentType))
                {
                    ...
                    List<WebhookResultDto> obj;
                    try
                    {
                        obj = ResponseBodyDeserializer.DeserializeNotNull<List<WebhookResultDto>>(httpResponseBody, NullValueHandling.Ignore);
                    }
                    catch (Exception ex)
                    {
                        throw new ResponseValidationException("Failed to deserialize response body into List<WebhookResultDto>.", httpRequest, httpResponse, httpResponseBody, ex);
                    }

                    var response = new InboundWebhooksControllerHandleWebhookResponse()
                    {
                    ...
                    response.WebhookResultDtos = obj;
                    return response;
                }
