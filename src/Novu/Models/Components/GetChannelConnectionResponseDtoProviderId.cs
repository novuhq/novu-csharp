...
namespace Novu.Models.Components
{
    ...
    public static class GetChannelConnectionResponseDtoProviderIdExtension
    {
        public static string Value(this GetChannelConnectionResponseDtoProviderId value)
        {
            return ((JsonPropertyAttribute)value.GetType().GetMember(value.ToString())[0].GetCustomAttributes(typeof(JsonPropertyAttribute), false)[0]).PropertyName ?? value.ToString();
        }

        public static GetChannelConnectionResponseDtoProviderId ToEnum(this string value)
        {
            foreach(var field in typeof(GetChannelConnectionResponseDtoProviderId).GetFields())
            {
                ...
                if (attribute != null && attribute.PropertyName == value)
                {
                    ...
                    if (enumVal is GetChannelConnectionResponseDtoProviderId)
                    {
                        return (GetChannelConnectionResponseDtoProviderId)enumVal;
                    }
                }
            }

            throw new Exception($"Unknown value {value} for enum GetChannelConnectionResponseDtoProviderId");
        }
    }
}